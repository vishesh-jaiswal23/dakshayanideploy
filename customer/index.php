...
if (($_SESSION['user_role'] ?? null) !== 'customer') {
    header('Location: /login.php');
    exit;
}

$displayName = $_SESSION['display_name'] ?? 'Customer';
...
<html lang="en">
  ...
  <body class="min-h-screen bg-slate-100 text-slate-800">
    <div class="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-6 px-6 py-12">
      <header class="text-center">
        <h1 class="text-3xl font-semibold text-slate-900">Customer Dashboard</h1>
        <p class="mt-2 text-sm text-slate-500">Here you can track projects, subsidies, and service updates.</p>
      </header>
      ...
      <section class="grid gap-6 md:grid-cols-2">
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <dl class="mt-4 space-y-2 text-sm text-slate-600">
            ...
            <div class="flex justify-between gap-3"><dt class="font-medium text-slate-500">Role</dt><dd>Customer</dd></div>
          </dl>
        </article>

        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 class="text-lg font-semibold text-slate-900">Next steps</h2>
          <p class="mt-2 text-sm text-slate-500">This area is ready for project timelines, status widgets, and customer support shortcuts.</p>
        </article>
      </section>
