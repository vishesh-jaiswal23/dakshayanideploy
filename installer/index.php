...
if (($_SESSION['user_role'] ?? null) !== 'installer') {
    header('Location: /login.php');
    exit;
}

$displayName = $_SESSION['display_name'] ?? 'Installer';
...
<html lang="en">
  <head>
    ...
    <title>Installer Portal | Dakshayani Enterprises</title>
    <link href="https://cdn.jsdelivr.net/npm/tailwindcss@3.4.4/dist/tailwind.min.css" rel="stylesheet" />
  </head>
  <body class="min-h-screen bg-slate-100 text-slate-800">
    <div class="mx-auto flex min-h-screen w-full max-w-4xl flex-col gap-6 px-6 py-12">
      <header class="text-center">
        <h1 class="text-3xl font-semibold text-slate-900">Installer Workspace</h1>
        <p class="mt-2 text-sm text-slate-500">Welcome back, <?= htmlspecialchars($displayName, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?>.</p>
      </header>

      <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-semibold text-slate-900">Account overview</h2>
        <dl class="mt-4 space-y-2 text-sm text-slate-600">
          <div class="flex justify-between gap-3"><dt class="font-medium text-slate-500">Role</dt><dd>Installer</dd></div>
        ...
        <p class="mt-4 text-sm text-slate-500">
          This is a placeholder dashboard. Build installer-specific tools and reports here.
        </p>
      </section>
